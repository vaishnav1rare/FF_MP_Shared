...
namespace OneRare.FoodFury.Multiplayer
{
    ...
    public class LevelManager : NetworkSceneManagerDefault
    {
        ...
        [FormerlySerializedAs("_scoreManager")] [Header("General Settings")] [SerializeField]
        private ScoreManager scoreManager;

        [FormerlySerializedAs("_countdownManager")] [SerializeField]
        private CountdownManager countdownManager;

        [FormerlySerializedAs("_lobby")] [SerializeField]
        private int lobby;
...
//			_scoreManager.ResetAllGameScores();
        ...
        public int GetRandomLevelIndex()
        {
            ...
            if (levels[idx] == _loadedScene.AsIndex)
        ...
        public void LoadLevel(int nextLevelIndex)
        {
            ...
            if (nextLevelIndex < 0)
            {
                Runner.LoadScene(SceneRef.FromIndex(lobby), new LoadSceneParameters(LoadSceneMode.Additive));
            }
            else
            {
        ...
        protected override IEnumerator OnSceneLoaded(SceneRef newScene, Scene loadedScene,
            NetworkLoadSceneParameters sceneFlags)
        {
            ...
            if (gameManager.MatchWinner != null && newScene.AsIndex == lobby)
            {
                // Show lobby scores and reset the score ui.
                scoreManager.ShowFinalGameScore(gameManager);
            }
            ...
            foreach (FusionPlayer fusionPlayer in gameManager.AllPlayers)
            {
                Player player = (Player)fusionPlayer;
                Debug.Log(
                    $"Initiating Respawn of Player #{fusionPlayer.PlayerIndex} ID:{fusionPlayer.PlayerId}:{player}");
                player.Reset();
                player.Respawn();
                yield return new WaitForSeconds(0.3f);
            }
...
//		    Debug.Log($"Switched Scene from {prevScene} to {newScene}");
        ...
        {
            var point = _challengeManager.orderSpawnPoints[0];
