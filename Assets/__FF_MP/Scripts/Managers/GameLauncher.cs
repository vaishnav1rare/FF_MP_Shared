...
public class GameLauncher : MonoBehaviour, INetworkRunnerCallbacks
{
	...
	private void Start()
	{
		...
		GlobalManager.Instance.GameLauncher = this;
		//SceneManager.LoadScene(LevelManager.LOBBY_SCENE);
	}

	
	public void JoinOrCreateLobby(GameMode gameMode, string roomName)
	{
		_gameMode = gameMode;
		...
		{
			GameMode = _gameMode,
			SessionName = roomName,//_gameMode == GameMode.Host ? ServerInfo.LobbyName : ClientInfo.LobbyName,
			//ObjectProvider = _pool,
			SceneManager = _levelManager,
			PlayerCount = 4,//ServerInfo.MaxUsers,
			EnableClientSessionCreation = false
		});
	}

	public void JoinRandomRoom(GameMode gameMode)
	{
		...
		{
			GameMode = _gameMode,
			SessionName = string.Empty, //_gameMode == GameMode.Host ? ServerInfo.LobbyName : ClientInfo.LobbyName,
			//ObjectProvider = _pool,
			SceneManager = _levelManager,
			PlayerCount = 4,//ServerInfo.MaxUsers,
			EnableClientSessionCreation = true
		});
		ServerInfo.LobbyName = "OneRare";
	}
